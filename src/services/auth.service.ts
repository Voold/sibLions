...
export const getFullUserInfoFromTpu = async (
  tokenType: string,
  accessToken: string,
) => {
  ...
  try {
    const [userRes, studyRes] = await Promise.all([
      tpuApi.get(getOauthConfig.getUserInfoEndpoint, {
        headers: { Authorization: authHeader },
      }),
      tpuApi.get(getOauthConfig.getUserStudyInfoEndpoint, {
        headers: { Authorization: authHeader },
      }),
    ]);
...
export const transformTpuToNewUser = (tpuRawData: any): NewUser => {
  ...
  const mainStudy =
    studyInfo.data?.studies?.find((s: any) => s.record_book_number) ||
